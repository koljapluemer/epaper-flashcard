...
Within this web app, follow a feature-sliced design structure, *not* the standard folder-by-type vue comes with.
...
- `entities`: models/entities. One folder per user-space entity such as "flashcard". May only import from `dumb`
...
- `pages`: One folder per page (a page is something used by the `router.ts` file). If functionality is ONLY used on a given page, put it in the page *folder* (avoid having just a single giant page file, split it up!), do not create features that are only used by one single page.
