...
// Wraps text word-by-word; returns line count drawn.
int drawWrapped(const char* text, int x, int y, int maxW, uint8_t sz, int maxLines) {
  ...
  while (tok && drawn < maxLines) {
    ...
    if (line[0] == '\0') {
      ...
      snprintf(candidate, sizeof(candidate), "%s %s", line, tok);
    }
    ...
    if (line[0]) tok = strtok_r(nullptr, " ", &save);
    else          tok = strtok_r(nullptr, " ", &save);
  }
...
void renderCard() {
  const char* country = cards[currentIdx].q;
  const char* capital = cards[currentIdx].a;
  ...
  do {
    ...
    // Header bar
    uint16_t barColor = showAnswer ? GxEPD_YELLOW : GxEPD_RED;
    display.fillRect(0, 0, 200, 28, barColor);
    display.setTextColor(GxEPD_BLACK);
    display.setTextSize(1);
    display.setCursor(6, 10);
    display.print(showAnswer ? "Answer:" : "Capital of:");

    // Main text (country name)
    display.setTextColor(GxEPD_BLACK);
    uint8_t qSz = (strlen(country) <= 10) ? 3 : 2;
    drawWrapped(country, 6, 40, 188, qSz, 3);

    // Answer (if revealed)
    if (showAnswer) {
      display.drawFastHLine(6, 118, 188, GxEPD_BLACK);
      uint8_t aSz = (strlen(capital) <= 10) ? 3 : 2;
      drawWrapped(capital, 6, 126, 188, aSz, 3);
    }
    ...
    display.print(showAnswer ? "press for next" : "press to reveal");
...
void setup() {
  ...
  digitalWrite(EPD_PWR, LOW);  // active-LOW: LOW = display on
  delay(100);
...
void loop() {
  ...
  static bool lastLogged = HIGH;
  if (pressed != lastLogged) {
    Serial.printf("GPIO1: %s\n", pressed ? "LOW (pressed)" : "HIGH (released)");
    lastLogged = pressed;
  }
